...
var url = 'http://movie.douban.com/subject/1292052/';
        ...
        if (!err && res.statusCode == 200) {
            ...
            async.waterfall([
                ...
                for (var i = 0; i < links.length; i++) {
                    timerList['timer' + i] = {};
                    timerList['timer' + i] = setTimeout(function() {
                        var endTime = new Date();
                        console.log('waiting seconds: ' + (endTime - startTime));
                        getPageData(links[this.index]);
                    }, 1000 * Math.ceil(Math.random() * 30 + 20));
                    timerList['timer' + i].index = i;
                }
